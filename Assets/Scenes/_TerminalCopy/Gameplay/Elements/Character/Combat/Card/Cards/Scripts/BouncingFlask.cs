...
public partial class BouncingFlask : Card {
    int initialBounceAmount = 3;
    int upgradedBounceAmount = 4;
    int BounceAmount => IsUpgraded ? upgradedBounceAmount : initialBounceAmount;
    ...
    public override string GetDescription(CharacterCombat caster, CharacterCombat target) {
        return $"Apply 3 {BrownText("Poison")} to a random enemy {NumberText(initialBounceAmount, BounceAmount)} times.";
    }
}
